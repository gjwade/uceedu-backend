...
module.exports = {
	...
	main: async (event) => {
		...
		if (await businessState.hasBuyFreeLiveCourses(event, data._id, userInfo)) {
			res.code = -1
			res.msg = "您已订购该课程"
			return res;
		}
		...
		let type = applicationConstant.FERR_LIVE_COURSE
		...
		let goods_item = [{
			...
			'type': type,
			'num': 1,
			'goods_name': "[公开课]" + course['title'],
			'goods_thumb': course['avatar'],

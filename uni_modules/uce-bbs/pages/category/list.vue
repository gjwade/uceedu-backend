	...
	export default {
		data() {
			...
			return {
				...
				table1: {
					// 表格数据请求地址
					action: "admin/bbs/category/sys/getList",
					// 表格字段显示规则
					columns: [
		...
		methods: {
			...
			addBtn() {
				that.resetForm();
				that.form1.props.action = 'admin/bbs/category/sys/add';
			...
			// 显示修改页面
			updateBtn({ item }) {
				that.form1.props.action = 'admin/bbs/category/sys/update';
			...
			deleteBtn({ item, deleteFn }) {
				deleteFn({
					action: "admin/bbs/category/sys/delete",
					data: {
						_id: item._id
					},
				});

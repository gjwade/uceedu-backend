	...
	export default {
		...
		data() {
			...
			return {
				...
				table1: {
					// 表格数据请求地址
					action: "admin/q_bank/bank_chapter/sys/getList",
					// 表格字段显示规则
					columns: [
		...
		methods: {
			...
			addBtn() {
				that.resetForm();
				that.form1.props.action = 'admin/q_bank/bank_chapter/sys/add';
			...
			// 显示修改页面
			updateBtn({ item }) {
				that.form1.props.action = 'admin/q_bank/bank_chapter/sys/update';
			...
			deleteBtn({ item, deleteFn }) {
				deleteFn({
					action: "admin/q_bank/bank_chapter/sys/delete",
					data: {
						_id: item._id
					},
				});

	...
	export default {
		data() {
			...
			return {
				...
				table1: {
					// 表格数据请求地址
					action: "admin/operations/slider/sys/getList",
					// 表格字段显示规则
					columns: [
						{ key: "_id", title: "幻灯片ID", type: "text", width: 220 },
						{ key: "thumb", title: "幻灯片", type: "image", width: 120 },
		...
		methods: {
			...
			addBtn() {
				that.resetForm();
				that.form1.props.action = 'admin/operations/slider/sys/add';
			...
			// 显示修改页面
			updateBtn({ item }) {
				that.form1.props.action = 'admin/operations/slider/sys/update';
			...
			deleteBtn({ item, deleteFn }) {
				deleteFn({
					action: "admin/operations/slider/sys/delete",
					data: {
						_id: item._id
					},
				});

	...
	export default {
		...
		data() {
			...
			return {
				...
				table1: {
					// 表格数据请求地址
					action: "admin/video_course/chapter/sys/getList",
					// 表格字段显示规则
					columns: [
						{ key: "_id", title: "章节ID", type: "text", width: 220 },
						{ key: "title", title: "章节名", type: "text", width: 180 },
						{ key: "sort", title: "排序", type: "text", width: 100, sortable: "custom" },
				...
				queryForm1: {
					...
					columns: [
						...
						{ key: "title", title: "章节名", type: "text", width: 160, mode: "%%" },
						{ key: "create_date", title: "添加时间", type: "datetimerange", width: 400, mode: "[]" },
					]
				},
				form1: {
					"data": {
		...
		methods: {
			...
			addBtn() {
				that.resetForm();
				that.form1.props.action = 'admin/video_course/chapter/sys/add';
			...
			// 显示修改页面
			updateBtn({ item }) {
				that.form1.props.action = 'admin/video_course/chapter/sys/update';
			...
			deleteBtn({ item, deleteFn }) {
				deleteFn({
					action: "admin/video_course/chapter/sys/delete",
					data: {
						_id: item._id
					},
				});

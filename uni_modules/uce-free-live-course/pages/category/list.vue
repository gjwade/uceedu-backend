	...
	export default {
		data() {
			...
			return {
				...
				table1: {
					// 表格数据请求地址
					action: "admin/free_live_course/category/sys/getList",
					// 表格字段显示规则
					columns: [
						{ key: "_id", title: "分类ID", type: "text", width: 220 },
						{ key: "name", title: "分类名", type: "text", width: 180 },
						{ key: "is_show", title: "是否显示", type: "switch", width: 80 },
				...
				queryForm1: {
					...
					columns: [
						{ key: "name", title: "分类名", type: "text", width: 160, mode: "%%" },
		...
		methods: {
			...
			addBtn() {
				that.resetForm();
				that.form1.props.action = 'admin/free_live_course/category/sys/add';
			...
			// 显示修改页面
			updateBtn({ item }) {
				that.form1.props.action = 'admin/free_live_course/category/sys/update';
			...
			deleteBtn({ item, deleteFn }) {
				deleteFn({
					action: "admin/free_live_course/category/sys/delete",
					data: {
						_id: item._id
					},
				});
